...
public class TemplateController {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(summary = "Archive a template so it doesn't show in the active list, but preserves old reports")
    public ResponseEntity<ApiResponse<Void>> archiveTemplate(
        @PathVariable Long templateId,
        @RequestParam(defaultValue = "SYSTEM") String archivedBy) {

        templateService.archiveTemplate(templateId, archivedBy);
        return ResponseEntity.ok(ApiResponse.success(null, "Template archived successfully"));
    }
